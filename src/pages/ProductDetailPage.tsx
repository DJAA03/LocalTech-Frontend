...
import { getProductById, getResenas, createResena, getRecomendaciones } from '../services/api';
...
export const ProductDetailPage: React.FC = () => {
    ...
    const [recomendaciones, setRecomendaciones] = useState<Product[]>([]);
    ...
    useEffect(() => {
        const fetchData = async (productId: string) => {
            ...
            setError(null);
            try {
                const [productRes, resenasRes, recomendacionesRes] = await Promise.all([
                    getProductById(productId), 
                    getResenas(productId),
                    getRecomendaciones(productId)
                ]);
    ...
    return (
        <div className="container mx-auto p-6 lg:p-12">
            <Link to="/" className="flex items-center gap-2 text-sm text-gray-600 hover:text-primary mb-8"><ChevronLeft size={16} />Volver a la tienda</Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                <div className="space-y-6">
                    <div>
                        <span className="text-sm font-semibold text-primary uppercase">{typeof product.categoria === 'object' ? product.categoria.nombre : ''}</span>
                    ...
                    <div className="bg-gray-50 p-6 rounded-lg space-y-4">
                        ...
                        {isOutOfStock ? 
                            <button disabled className="w-full bg-red-600 text-white font-bold py-3 rounded-lg cursor-not-allowed">Agotado</button> 
                            : 
                            <button onClick={handleAddToCart} className="w-full bg-primary text-white font-bold py-3 rounded-lg hover:bg-primary-hover transition-transform transform hover:scale-105">
                                <ShoppingCart className="inline-block mr-2" />Añadir al Carrito
                            </button>
                        }
                    </div>
                </div>
            ...
            {recomendaciones.length > 0 && (
                <div className="mt-20 pt-10 border-t">
                    <h2 className="text-3xl font-bold text-gray-800 mb-8">También te podría interesar</h2>
                        ...
                        <form onSubmit={handleResenaSubmit} className="space-y-4">
                                ...
                                <div className="flex">
                                    {[1, 2, 3, 4, 5].map(star => (
                                        <Star 
                                            key={star} 
                                            size={28} 
                                            className={`cursor-pointer ${(hoverRating || newRating) >= star ? 'text-yellow-400' : 'text-gray-300'}`} 
                                            fill="currentColor" 
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder={`¿Qué te pareció ${product.nombre}?`}
                                />
                            </div>
