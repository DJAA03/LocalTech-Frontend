                    ...
                    <nav className="p-4">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-gray-300">OPCIONES</h3>
                             <button onClick={handleClearFilters} className="text-xs text-red-400 hover:underline flex items-center gap-1">
                        ...
                        <form onSubmit={handlePriceFilter} className="space-y-2 mb-6">
                            ...
                            <button type="submit" className="w-full text-sm bg-primary hover:bg-primary-hover text-white font-semibold py-2 rounded-md">
                                Aplicar Precio
                            </button>
                        </form>
                            ...
                            <div className="flex flex-col space-y-1">
                                {[4, 3, 2, 1].map((rating) => (
                                    <button 
                                        ...
                                        className={`w-full flex items-center gap-2 p-2 rounded-md text-sm transition-colors ${minRating === rating ? 'bg-primary' : 'hover:bg-gray-700'}`}
                                    >
                                        <div className="flex">
                        ...
                        <ul className="space-y-2">
                            <li>
                                <button 
                                    ...
                                    className={`w-full text-left flex items-center gap-3 p-3 rounded-lg transition-colors ${!selectedCategory ? 'bg-primary' : 'hover:bg-gray-700'}`}
                                >
                                    <List size={20} />
                            ...
                            {categories.map(cat => (
                                <li key={cat._id}>
                                    <button 
                                        ...
                                        className={`w-full text-left p-3 rounded-lg transition-colors ${selectedCategory === cat._id ? 'bg-primary' : 'hover:bg-gray-700'}`}
                                    >
                                        {cat.nombre}
                                    </button>
                                </li>
