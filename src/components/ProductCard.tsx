...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const shortDescription = product.descripcion.length > DESCRIPTION_MAX_LENGTH
    ? product.descripcion.substring(0, DESCRIPTION_MAX_LENGTH) + '...'
    : product.descripcion;
  ...
  return (
    <Link to={`/producto/${product._id}`} className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 flex flex-col relative group">
        ...
        <div className="p-5 flex flex-col flex-grow">
            ...
            {}
            <p className="text-gray-600 mt-2 text-sm flex-grow min-h-[60px]">
                {shortDescription}
            </p>
        </div>
        <div className="p-5 pt-0 mt-auto flex justify-between items-center">
            ...
            {isOutOfStock ? (
                ...
                <button onClick={handleAddToCart} className="bg-primary text-white font-bold py-2 px-4 rounded-full hover:bg-primary-hover transition-all duration-200 transform hover:scale-105 active:scale-95 flex items-center gap-2 z-20">
