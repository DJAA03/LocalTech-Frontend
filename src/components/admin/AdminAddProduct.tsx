...
import { createProduct, getCategories, generateProductDescription } from '../../services/api';
...
export const AdminAddProduct: React.FC = () => {
    ...
    const handleGenerateDescription = async () => {
        if (!nombre || !categoria) {
            Swal.fire('Atención', 'Por favor, ingresa el nombre y selecciona una categoría antes de generar la descripción.', 'warning');
            return;
        }
        setIsGenerating(true);
        try {
            const selectedCategoryName = categories.find(c => c._id === categoria)?.nombre || '';
            const res = await generateProductDescription(nombre, selectedCategoryName);
    ...
    const uploadImage = async (): Promise<string | null> => {
        ...
        if (!cloudName || !uploadPreset) {
            ...
            Swal.fire('Error de Configuración', 'Las credenciales de Cloudinary no están definidas. Revisa tu archivo .env.', 'error');
            return null;
        }
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} required rows={4} className="w-full p-2 border border-gray-300 rounded-md shadow-sm"/>
                </div>
                ...
                <button type="submit" disabled={isUploading || isGenerating} className="w-full bg-primary text-white py-2 rounded-md hover:bg-primary-hover disabled:bg-gray-400 transition-colors">
