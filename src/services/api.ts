...
export const getRecomendaciones = (productId: string) => api.get(`/productos/${productId}/recomendaciones`); 
...
export const generateProductDescription = (nombre: string, categoria: string) => api.post('/ia/generate-description', { nombre, categoria });
export const sendChatMessage = (history: any[], message: string) => api.post('/ia/chat', { history, message });
export const semanticSearch = (query: string) => api.get(`/ia/search?query=${encodeURIComponent(query)}`);
export const predictiveSearch = (query: string) => api.get(`/productos/predictive-search?query=${encodeURIComponent(query)}`);